...
> **Not in use yet.** The column runs one permanent painting every Monday, and
> the site forces it. This file is the specification for the per-week covers we
> will generate from a local ComfyUI workflow when that is built; it is kept
> here because it is the visual identity, and reconstructing it later from
> nothing would be the expensive part.
>
> Turning it on again is one revert: `git revert 26b10bf` restores the site
> change, the routine's fourth deliverable and the brief folder.
...
Every line below is a constraint, not a suggestion — the point of a column cover
is that twelve of them in a row look like a set.
...
Until then every Monday uses that permanent painting, and the site forces it
over whatever the frontmatter says.
