# Source feeds — harvested 2026-09-08T19:35:03.417Z

Fetched by GitHub Actions, which is not behind the writer's egress proxy.
One file per category. Each item is what the publisher syndicates in its
own feed: headline, link, date, and their summary.

> **Every draft needs `sources:` with two different outlets.** Not two
> links to the same publication, which is one source read twice. If only
> one outlet has the story, write it, keep `draft: true`, and say so in
> the first line of the body. See notes/article-voice.md.

> **A dated showcase gets two articles, not one.** A preview before it
> airs, carrying the stream link and the start time in Italian time, and a
> round-up an hour after it ends. Directs, State of Play, Xbox showcases,
> TGA, gamescom ONL, TGS. See notes/event-coverage.md and keep its
> watchlist current.

## Editorial mix — what the archive owes

**Write Manga and anime next.** 48 articles have gone out since the last one.

- **Manga and anime** — BROKEN. 48 articles since the last one; quota is one in 6. 3 in the archive of 137.
- **Film & TV** — BROKEN. 54 articles since the last one; quota is one in 8, never worse than one in 10. 10 in the archive of 137.

The run is what counts, not the percentage: a burst last month does not
excuse thirty in a row without one. If a category is owed and the feeds
hold nothing worth writing about, say so in the draft rather than
silently skipping it.

## Feeds

- [Manga](manga.md) **owed** — 35 items
- [Film & TV](film-tv.md) **owed** — 67 items
- [Games](games.md) — 96 items
- [Tech](tech.md) — 77 items
- [3D](3d.md) — 16 items
- [AI](ai.md) — 9 items

Total: 300 items.

Feeds that did not answer: VentureBeat AI (HTTP 429).
