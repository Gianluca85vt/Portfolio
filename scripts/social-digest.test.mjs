/**
 * The two pieces of the daily story that decide whether it runs and whether
 ...
 */
import { isDue, recordStory } from './social-digest.mjs';
...
const empty = {};
const postedToday = { 'some-slug': { storyAt: '2026-09-03' } };
const postedYesterday = { 'some-slug': { storyAt: '2026-09-02' } };
...
check('entries with no digest yet are ignored', isDue({ a: { facebook: '1' }, b: null }, new Date('2026-09-03T17:00:00Z')).due, true);

/* -------------------------------------------------------- recordStory ---
 *
 * 3 September: the digest posted and the ledger went back to disk unchanged,
...
const after = recordStory(before, batch, ['IG_1', 'IG_2'], '2026-09-03');

check('the posted article carries every frame id', after['went-out-tonight'].story, 'IG_1,IG_2');
check('and the day it went out', after['went-out-tonight'].storyAt, '2026-09-03');
...
check('an id from the carousel era is left alone', after['already-done'].carousel, 'old');
...
check('the input is left alone', before['went-out-tonight'].story, undefined);

// A slug with no prior record still gets one.
check(
  'a slug absent from the ledger is added',
  recordStory({}, [{ slug: 'brand-new' }], ['IG_9'], '2026-09-03')['brand-new'],
  { story: 'IG_9', storyAt: '2026-09-03' }
);
